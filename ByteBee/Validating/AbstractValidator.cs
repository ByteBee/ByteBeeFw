...
namespace ByteBee.Validating
{
    public abstract class AbstractValidator<TObject> : IValidator<TObject>, IValidatorExceptional<TObject>
    {
        ...
        public void ValidateAndThrow(TObject obj)
        {
            ...
            if (!result.IsValid)
            {
                throw new ObjectNotValidException(result.ToString());
            }
        }

        ///// <inheritdoc />
        //public IValidator<TObject> RuleSet<TOther>(AbstractValidator<TOther> other, TOther obj, string message)
        //{
        ...
        public IValidator<TObject> Include(AbstractValidator<TObject> other)
        {
